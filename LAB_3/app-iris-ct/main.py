...
class TrainRequest(BaseModel):
    ...
    policy: Optional[str] = "any_improvement"
...
class ActivationPolicy:
    def __init__(self, mode="any_improvement", delta=0.0, target_class=None):
        self.mode = mode
        self.delta = delta
        self.target_class = target_class

    def should_activate(self, previous_accuracy, accuracy_new, previous_f1=None, new_f1=None):

        if previous_accuracy is None:
            return True, "No existe modelo previo"

        if self.mode == "any_improvement":
            if accuracy_new >= previous_accuracy:
                return True, "Accuracy nuevo >= anterior"
            else:
                return False, "Accuracy nuevo < anterior"

        elif self.mode == "min_delta":
            if accuracy_new >= previous_accuracy + self.delta:
                return True, "Mejora suficiente (>= delta)"
            else:
                return False, "Mejora insuficiente"

        elif self.mode == "per_class_f1":
            if previous_f1 is None or new_f1 is None:
                return False, "No hay métricas F1 disponibles"

            if new_f1[self.target_class] > previous_f1[self.target_class]:
                return True, "F1 de clase objetivo mejora"
            else:
                return False, "F1 de clase objetivo no mejora"
...
def train(request: TrainRequest):
    ...
    policy = ActivationPolicy(mode=request.policy)

    model_updated, reason = policy.should_activate(
        previous_accuracy,
        accuracy_new
    )
